...
# 기본 경로
DEFAULT_JSON_DIR = r"C:\Users\Yul\PycharmProjects\PitcherKinetiX\data\json_2d"
DEFAULT_OUTPUT_DIR = r"C:\Users\Yul\PycharmProjects\PitcherKinetiX\data\processed"
...
def run_preprocessing(
        ...
        verbose: bool = True):
    """
    각 JSON 파일을 개별적으로 처리하고
    window + video_id + filename 저장.
    """
    ...
    for vid, file in enumerate(file_list):
        ...
        # ------------------------
        # 1) keypoint preprocess
        # ------------------------
        kps = processor.process_file(json_path)  # (240, 17, 2)
...
def combine_processed(output_dir: str, save_filename="2d_data.npz"):
    """
    모든 *_processed.npz 파일을 합쳐서
    - windows
    - video_ids
    - filenames
    를 통합한 하나의 dataset으로 저장.
    """
    ...
    processed_files = sorted([f for f in os.listdir(output_dir)
