...
VAL_VIDEO_DIR = os.path.join(TRAIN_DATA_DIR, "val_video_data")        # 원본 영상
VAL_CROPPED_VIDEO_DIR = os.path.join(TRAIN_DATA_DIR, "val_cropped_videos")    # 투수만 crop한 영상
VAL_KPS_DIR = os.path.join(TRAIN_DATA_DIR, "val_kps")            # keypoint JSON
VAL_JSON_2D_DIR = os.path.join(TRAIN_DATA_DIR, "val_json_2d")           # 전처리 전 데이터
VAL_PROCESSED_DIR = os.path.join(TRAIN_DATA_DIR, "val_processed")            # 전처리 후 데이터